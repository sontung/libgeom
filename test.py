...
def loop_subdivision(vertices, triangles, save_name):
    ...
    for x, y, z in triangles:
        ...
        # hash edge
        edge_key = edge_hash(vertices[x], vertices[y])
        ...
        edge_key = edge_hash(vertices[y], vertices[z])
        if edge_key not in edge2id:
        ...
        edge_key = edge_hash(vertices[z], vertices[x])
        if edge_key not in edge2id:
    ...
    for v0, v1, v2 in triangles:
        edge_key1 = edge_hash(vertices[v0], vertices[v1])
        edge_key2 = edge_hash(vertices[v1], vertices[v2])
        edge_key3 = edge_hash(vertices[v2], vertices[v0])
        u8 = edge2id[edge_key1][-1]
        u5 = edge2id[edge_key2][-1]
        u7 = edge2id[edge_key3][-1]

        new_faces.append([u8, u5, u7])
        new_faces.append([u7, v0, u8])
        new_faces.append([u5, u8, v1])
        new_faces.append([v2, u7, u5])
    ...
    with open(save_name, 'w') as file:
        for vid in range(new_vert_ind):
            if vid >= vertices.shape[0]:
                ...
                print("v %f %f %f" % (new_vert2id[vid][0], new_vert2id[vid][1], new_vert2id[vid][2]), file=file)
            if vid < vertices.shape[0]:
                vertices2[vid] = [vertices[vid][0], vertices[vid][1], vertices[vid][2]]
                print("v %f %f %f" % (vertices[vid][0],
                                      vertices[vid][1],
                                      vertices[vid][2]), file=file)
        tid = 0
        for x, y, z in triangles:
